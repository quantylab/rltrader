...
if __name__ == '__main__':
    ...
    parser.add_argument('--action', choices=['train', 'test', 'update', 'predict'], default='train')
    ...
    # 파라미터 설정
    output_name = f'{args.action}_{args.name}_{args.rl_method}_{args.net}'
    learning = args.action in ['train', 'update']
    reuse_models = args.action in ['test', 'update', 'predict']
    ...
    start_epsilon = args.start_epsilon if args.action in ['train', 'update'] else 0
    num_epoches = args.num_epoches if args.action in ['train', 'update'] else 1
    ...
    if args.action in ['train', 'test', 'update']:
        learner.run(learning=learning)
        if args.action in ['train', 'update']:
            learner.save_models()
