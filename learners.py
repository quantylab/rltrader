...
class ReinforcementLearner:
    ...
    def visualize(self, epoch_str, num_epoches, epsilon):
        ...
        self.visualizer.plot(
            ...
            outvals_value=self.memory_value, outvals_policy=self.memory_policy,
            exps=self.memory_exp_idx, learning_idxes=self.memory_learning_idx,
            initial_balance=self.agent.initial_balance, pvs=self.memory_pv,
        )
    ...
    def run(
        ...
        for epoch in range(num_epoches):
            ...
            logging.info("[{}][Epoch {}/{}] Epsilon:{:.4f} #Expl.:{}/{} "
                        ...
                        "POS:{} NEG:{} Loss:{:.6f} ET:{:.4f}".format(
                            ...
                            self.pos_learning_cnt, self.neg_learning_cnt, self.loss, elapsed_time_epoch))
        ...
        # 학습 관련 정보 로그 기록
        logging.info("[{code}] Elapsed Time:{elapsed_time:.4f} Max PV:{max_pv:,.0f} #Win:{cnt_win}".format(
...
class A2CLearner(ActorCriticLearner):
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        ...
        for i, (sample, action, value, policy, reward) in enumerate(memory):
            ...
            advantage = r + discount_factor * value_mean_next - np.mean(value)
