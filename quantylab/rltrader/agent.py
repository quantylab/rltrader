...
class Agent:
    ...
    def __init__(self, \
        environment, balance, min_trading_unit=1, max_trading_unit=2):
        # Environment 객체
    ...
    def get_states(self):
        ...
        return (
            self.ratio_hold,
            self.ratio_portfolio_value,
            (self.environment.get_price() / self.avg_buy_price) -
            1 if self.avg_buy_price > 0 else 0
        )
    ...
    def act(self, action, confidence):
        ...
        if action == Agent.ACTION_BUY:
            ...
            invest_amount = curr_price * \
                (1 + self.TRADING_CHARGE) * trading_unit
            if invest_amount > 0:
                self.avg_buy_price = (self.avg_buy_price * self.num_stocks +
                                      curr_price) / (self.num_stocks + trading_unit)  # 주당 매수 단가 갱신
